...
{     
 	    $selectedTime = $batch_time; //11:07
        $login_time=$_SESSION['Current_time_exam']; //11:37
        
        $Starttime_plus_duration = strtotime($selectedTime) + $exam_duration*60;
  	    $endTimeduration= date('H:i', $Starttime_plus_duration);
	    $durationexam=(strtotime($endTimeduration)-strtotime($login_time))/60;
}
else
{
...
if($_GET['exam_id']!="")
{
	...
	(int)$count=count($subject_id_explode);

	for($p=0;$p<$count;$p++)
	{	
	   
		$query_question .="( SELECT * FROM question WHERE question_status =1 AND c_id = '".$category_id."' AND s_c_id = '".$subcategory_id."' AND s_id = '".$subject_id_explode[$p]."' ORDER BY rand( ) LIMIT ".$show_question_limit[$p]." )" ;
		if($p <$count-1)
		{
											...
											<tr>
												<td style="vertical-align:top">						
													...
													<div class="col-md-6"> <span class="question"> Question No. <?php echo $i;?></span></div>
														<div class="col-md-6" style="float:right;margin-bottom:30px;"> <span class="question"> Marks: <?php echo $result['marks'];?></span></div>
													</div>
													<h4 class="quction"><p class="no_select"><?php echo $result['question'];?></p></h4>
								...
								<div class="awareness-view"><h4 class="awareness-view-hed"><small><?php echo constant('TI_LABEL_QUESTION_PALETTE');?></small></h4>
									<div class="col-md-12">
										<div class="number-plate">
											...
											for($r=0;$r<$count;$r++)
											{		
												$query_question1 .="( SELECT * FROM question WHERE question_status =1 AND c_id = '".$category_id."' AND s_c_id = '".$subcategory_id."' AND s_id = '".$subject_id_explode[$r]."' ORDER BY rand( ) LIMIT ".$show_question_limit[$r]." )" ;
												if($r <$count-1)
												{
