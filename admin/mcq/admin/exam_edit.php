...
if(isset($_POST['submit']) && $_POST['e_id']!="")
{	
	...
	if($_POST['center_id']!="")
	{
		...
		{
			while (list ($key, $val) = each ($center_id)) 
			{
				$centerid .= "$val,";
			}
		}
		$exam_center_id= substr($centerid,0,-1);
	}
	else
	{
		$exam_center_id=0;
...
$exam_end=mysql_prep($_POST["exam_date_end"]);	
	...
	if(mysqli_num_rows($query_select_exam)>0)
	{
		//$error .= constant('TI_EXAM_ERROR_ALLREADY_EXIT') ;
		 $exam_date= mysql_prep($_POST['exam_date']);
		 $exam_date_end= mysql_prep($_POST['exam_date_end']);
		if($exam_date!="" &&  $exam_date!="")
		{
		...
		$dobexplode1=explode("/",$exam_end);
		$dob_day1=$dobexplode1[1];
		$dob_month1=$dobexplode1[0];
		$dob_year1=$dobexplode1[2];
		$exam_date_end=$dob_year1.'-'.$dob_month1.'-'.$dob_day1;
	...
	$query_insert_exam = "update exam set category_id='{$category_id}', subcategory_id='{$subcategory_id}', subject_id='{$subject_id}',  exam_date='{$exam_date_from}',date_end='{$exam_date_end}', exam_duration='{$exam_duration}',neg_mark_status='{$neg_mark_status}',negative_marks='{$negative_marks}',passing_percentage='{$passing_percentage}',re_exam_day='{$re_exam_day}',terms_condition='{$terms_condition}',result_show_on_mail='{$result_show_on_mail}',	center_id='{$exam_center_id}',show_question='{$show_question}',sort_order='{$sort_order}',time_reduction='{$time_reduction}',center_id='{$center_id}' where e_id='".$_POST['e_id']."'";
...
if($_GET['e_id']!="")
{
	...
	if($result['exam_date']!="" &&  $result['date_end']!="")
	{
		$exam_date_explode=explode("-",$result['exam_date']);		
		$exam_date=$exam_date_explode[1]."/".$exam_date_explode[2]."/".$exam_date_explode[0];
		$exam_date_explode1=explode("-",$result['date_end']);		
		$exam_date_end=$exam_date_explode1[1]."/".$exam_date_explode1[2]."/".$exam_date_explode1[0];
	}
}
					...
					<div class="tab-pane active" id="add" style="padding: 5px">
						...
						<div class="padded">       
							<div class="control-group">
                                ...
                                <div class="controls">
									<select name="center_id" class="chzn-select" onChange="getState(this.value)">
									<option><?php //echo constant('TI_SELECT_CATEGORY');?>Select Center</option>  
										<?php $query_center=mysqli_query($con,"select * from center where center_status=1");
										while($result_center=mysqli_fetch_array($query_center))
										{
										?>
                                    	<option value="<?php echo $result_center['c_id'];?>" <?php if($result_center['c_id']==$result['center_id']){echo 'selected="selected"';}?>><?php echo ucfirst($result_center['centername']);?></option>
							...
							<div class="padded">       
							<div class="control-group">
                                <label class="control-label"><?php //echo constant('TI_LABEL_CATEGORY_NAME');?>Center Name</label>
                                <div class="controls">
						...
						<div class="padded">                   
                            <div class="control-group">
                                <label class="control-label"><?php //echo constant('TI_LABEL_SUB_CATEGORY_NAME');?>Select Course Level</label>
                                 <div class="controls" id="subcategorydiv">
									<select name="s_c_id" class="chzn-select">
									<option><?php //echo constant('TI_SELECT_CATEGORY_FIRST');?>Select Course Level</option>
                            ...
                            <div class="control-group">
                                <label class="control-label"><?php echo constant('TI_LABEL_SUBJECT_NAME');?></label>
                                 <div class="controls" id="subjectdiv">
									...
									$query_question_no=mysqli_query($con,"SELECT * FROM question WHERE c_id=".$result['category_id']." AND s_c_id=".$result['subcategory_id']." AND  question_status=1 AND s_id='".$row['s_id']."'");
						...
						<div class="padded">                   
                            <div class="control-group">
                                <label class="control-label">Exam Date Start</label>
                                <div class="controls">
						...
						<div class="padded">                   
                            <div class="control-group">
                                <label class="control-label">Exam Date End</label>
                                <div class="controls">
                                    <input type="text" name="exam_date_end"  class="validate[required] datepicker" value="<?php echo $exam_date_end;?>"/>
                                </div>
                            </div>
                        </div>
						...
						<div  id="div1">
							<div class="padded">                   
								<div class="control-group">
									...
									<div class="controls">
										<input type="text"  name="negative_marks" class="validate[custom[number]]" id="pr_departuretime_airlines" value="<?php echo $result['negative_marks'];?>"/>
									</div>
								</div>
							</div>
						</div>
