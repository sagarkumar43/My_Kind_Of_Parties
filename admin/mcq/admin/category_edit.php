...
{	
	...
	{
		$query_update_mysql_category=mysqli_query($con,"UPDATE category SET category_name='".$category_name."' where c_id='".$_POST['c_id']."'");

		if($query_update_mysql_category)
		{
			
			$message_success .=$category_name." ". constant('TI_CATEGORY_EDIT_MESSAGE');
		...
		<div class="row-fluid">
			<div class="area-top clearfix">
				<div class="pull-left header">
					<h3 class="title"><i class="icon-edit"></i><?php// echo constant('TI_MANAGE_CATEGORY');?>Manage Course </h3>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="box">
			<div class="box-header">    	
				<ul class="nav nav-tabs nav-tabs-left">
					<li class="active">
						<a href="#add" data-toggle="tab"><i class="icon-wrench"></i><?php //echo constant('TI_CATEGORY_EDIT_BUTTON');?> Edit Course</a>
					</li>
				</ul>	
			</div>
					...
					<div class="tab-pane active" id="add" style="padding: 5px">
						<form method="post" action="<?php $PHP_SELF ?>" class="form-horizontal validatable" enctype="multipart/form-data">
							...
							<div class="form-actions">
								<button type="submit" class="btn btn-gray"><?php echo constant('TI_CATEGORY_EDIT_BUTTON');?></button>
