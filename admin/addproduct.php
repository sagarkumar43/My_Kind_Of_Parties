...
foreach($_FILES["p_sliderimage"]["tmp_name"] as $key=>$tmp_name) {
    ...
    if(in_array($ext,$extension)) {
        if(!file_exists("img/product_img/slider_img/".$txtGalleryName."/".$file_name)) {
            move_uploaded_file($file_tmp=$_FILES["p_sliderimage"]["tmp_name"][$key],"img/product_img/slider_img/".$txtGalleryName."/".$file_name);
            $items[] =$file_name;
            ...
            move_uploaded_file($file_tmp=$_FILES["p_sliderimage"]["tmp_name"][$key],"img/product_img/slider_img/".$txtGalleryName."/".$newFileName);
            $items[] =$newFileName;
        }
    }
    else {
...
$product_managment="insert into product_mgmt set product_name='$pname',product_price='$pprice',product_desciption='$pdesc',product_inclusion='$pinclude',product_video='$video_path',need_to_know='$know',cancellation_policy='$cancel',feature_image='$filename1',slider_image='$multi_images',uploaded_by='Admin',category_name='$category'";

if(mysqli_query($conn, $product_managment)==true)
{
   echo "<script>alert('Products saved')</script>";
    echo "<script>window.location.href='manageproduct.php'</script>";
}
else
{
    echo "<script>alert('Products not saved')</script>";
    echo "<script>window.location.href='manageproduct.php'</script>";
}
}
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12">
                                     ...
                                     <textarea id="editor4" name="p_needtoknow"></textarea>
                                    <script>
